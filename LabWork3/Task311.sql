...
AS
    ...
    BEGIN
        FOR fk_rec IN fk_cur LOOP
            ...
            DBMS_OUTPUT.PUT_LINE('Current table in traversal is ' || current_table_name);
            IF next_table_name = start_table_name THEN
                loop_detected := TRUE;
                DBMS_OUTPUT.PUT_LINE('CIRCULAR DEPENDENCY DETECTED!!!: Start_table_name -> ' || start_table_name || ' End_table_name -> ' || current_table_name || ' depth:3');
                RETURN;
            ELSE
    ...
    BEGIN
        ...
        -- Проверяем, существует ли колонка в таблице из dev_schema_name также и в таблице из prod_schema_name
            IF column_exists_in_table(prod_col_rec.cl, table_name0, dev_schema_name) THEN
                DBMS_OUTPUT.PUT_LINE('Column ' || prod_col_rec.cl || ' exists in both PROD and DEV schemas.');
            ELSE
            -- Если колонка не существует в таблице из prod_schema_name, устанавливаем флаг в FALSE
                tables_equal := FALSE;
                DBMS_OUTPUT.PUT_LINE('Column ' || prod_col_rec.cl || ' exists in PROD schema but not in DEV schema.');
                RETURN tables_equal;
            END IF;
        END LOOP;
...
BEGIN
    ...
    FOR dev_rec IN dev_tables_cur LOOP
        ...
        DBMS_OUTPUT.PUT_LINE('TableInCycle: '|| dev_table_name);
        table_exists := FALSE;
